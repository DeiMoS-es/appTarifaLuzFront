    ...
    <div class="grid gap-6" *ngIf="view">
      ...
      <section aria-labelledby="forecast-title"><h2 id="forecast-title" class="mb-5 mt-4 text-3xl font-medium">Pronóstico</h2>
        <div class="grid grid-cols-2 gap-4">
          <article class="rounded-[18px] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,.06)]" style="border-top: 4px solid #10b981;" data-forecast-card><p class="m-0 text-sm uppercase text-slate-700">Media</p><p class="mb-2 mt-5 font-mono text-3xl font-bold">{{ view.forecast.average }}<small class="ml-1 text-sm font-medium">€/kWh</small></p><p class="m-0 text-sm font-semibold text-emerald-600" *ngIf="view.forecast.variation; else noComparison">{{ view.forecast.variation }}</p><ng-template #noComparison><p class="m-0 text-xs text-slate-500">Sin comparación con hoy</p></ng-template></article>
          <article class="rounded-[18px] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,.06)]" style="border-top: 4px solid #38bdf8;" data-forecast-card><p class="m-0 text-sm uppercase text-slate-700">Mínimo</p><p class="mb-2 mt-5 font-mono text-3xl font-bold">{{ view.forecast.minimum }}<small class="ml-1 text-sm font-medium">€/kWh</small></p><p class="m-0 text-sm text-slate-700">◷ {{ view.forecast.minimumTime }}</p></article>
        </div>
      </section>
      ...
      <section aria-labelledby="best-hours-title"><h2 id="best-hours-title" class="mb-5 mt-4 text-3xl font-medium">Mejores horas</h2>
        <ol class="m-0 grid list-none gap-3 p-0"><li *ngFor="let hour of view.bestHours; trackBy: trackBest" class="grid grid-cols-[1fr_auto] items-center gap-3 rounded-[16px] bg-white px-5 py-4 shadow-sm" [style.border-left]="'4px solid ' + (hour.band === 'low' ? '#38bdf8' : hour.band === 'medium' ? '#fbbf24' : '#ef4444')" data-best-hour><div><strong class="font-mono text-xl">{{ hour.range }}</strong><p class="mb-0 mt-1 text-sm text-slate-600">{{ hour.copy }}</p></div><div class="text-right"><strong class="font-mono text-lg">{{ hour.price }}€</strong><p class="mb-0 mt-2 inline-block rounded-full px-3 py-1 text-xs font-bold uppercase" [ngClass]="hour.band === 'low' ? 'bg-sky-100 text-sky-600' : hour.band === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'">{{ hour.bandLabel }}</p></div></li></ol>
      </section>
    </div>`,
